...
async def SendMonthlyResoconto(context: CallbackContext):
    columnsOperazioni = ['ID_Operazione', 'ID_Telegram', 'Username', 'Nome_Auletta', 'Prodotto', 'Costo', 'Pagato', 'Data_Ora']
    columnsRicariche = ['ID_Ricarica', 'ID_Beneficiario', 'Username_Beneficiario', 'ID_Amministratore', 'Username_Amministratore', 'Importo', 'Saldo_Prima', 'Saldo_Dopo', 'Date_Time_Ricarica']

    rowsOperazioni = GetOperazioniMensiliExcel()
    rowsRicariche = GetRicaricheMensiliExcel()
    print(rowsRicariche)
    ...
    await context.bot.send_document(chat_id=f"{GetChannelID()}", document=excel_buffer, filename=f'Resoconto-{d.day}-{d.month}.xlsx', caption=f"Resoconto mensile del {d.day}-{d.month} inviato!")
    excel_buffer.close()
