...
def GetProdotti(idAuletta : int) -> str:
    ...
    res2 = session.query(Magazzino, Prodotto).join(Prodotto, Prodotto.ID_Prodotto == Magazzino.ID_Prodotto, isouter=True).filter(Magazzino.ID_Auletta == f"{idAuletta}").all()
